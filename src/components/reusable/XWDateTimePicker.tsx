...
import { Dialog, DialogTrigger, DialogContent, DialogTitle } from "../reusable/xw-dialog";
...
export default function XWDateTimePicker({ onChange, value }: XWDateTimePickerProps) {
    ...
    const handleTimeChange = (updatedHours: string, updatedMinutes: string, updatedAmpm: string) => {
        if (date) {
            const dt = date
                .set({ hour: Number(updatedHours), minute: Number(updatedMinutes), second: 0 })
                .set({ hour: updatedAmpm === "AM" ? Number(updatedHours) : Number(updatedHours) + 12 });
            setDate(dt);
            onChange?.(dt.toJSDate());
        }
        ...
        <div>
            <Dialog open={open} onOpenChange={setOpen}>
                <DialogTrigger asChild>
                    <Button variant="xw_outline" className="w-full justify-start text-left">
                        <CalendarIcon className="mr-2 h-4 w-4" />
                        {date ? `${date.toFormat("DDD t")} ${ampm}` : <span>Pick date and time</span>}
                    </Button>
                </DialogTrigger>
                <DialogContent className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="flex items-center space-x-2 w-full">
                            {/* Hours */}
                            <Select value={hours} onValueChange={(val) => handleTimeChange(val, minutes, ampm)}>
                                <SelectTrigger >
                            ...
                            {/* Minutes */}
                            <Select value={minutes} onValueChange={(val) => handleTimeChange(hours, val, ampm)}>
                                <SelectTrigger>
                            ...
                            {/* AM/PM */}
                            <Select value={ampm} onValueChange={(val) => handleTimeChange(hours, minutes, val)}>
                                <SelectTrigger>
