...
const DialogClose = DialogPrimitive.Close // Adding the DialogClose component
...
const DialogContent = React.forwardRef<
    ...
    React.ComponentPropsWithoutRef<typeof DialogPrimitive.Content> & {
        maxWidth?: string;
        enableClose?: boolean; // New Prop
    }
>(({ className, children, maxWidth = 'sm', enableClose = true, ...props }, ref) => (
    <DialogPortal>
        <DialogOverlay />
        <DialogPrimitive.Content
            ...
            className={cn(
                ...
                `max-w-${maxWidth} w-full bg-xw-background border border-xw-secondary rounded-lg p-6 shadow-lg`,
                className
            )}
            {...props}
        >
...
>(({ className, ...props }, ref) => (
    <DialogPrimitive.Description
        ref={ref}
        className={cn('text-sm text-xw-muted mb-4', className)}
        {...props}
    />
));
...
export {
    ...
    DialogTrigger,
    DialogContent,
    DialogClose, // Exporting the DialogClose component
    DialogHeader,
    DialogTitle,
    DialogDescription,
};
