...
import { flowType } from "~/atoms/writerXAtoms";
...
export const DynamicForm = ({
  ...
  const [showNewOptionInput, setShowNewOptionInput] = useState<Record<string, boolean>>({});
  ...
  const [transcribedFields, setTranscribedFields] = useState<Record<string, boolean>>({});
  ...
  const handleUrlTranscription = async (type: string, url: string) => {
    ...
    try {
      ...
      const res = await handleFileTrasncription({ url: url, type: isYoutubeUrl ? 'youtube' : 'mux' });
  ...
  const handleAddNewOption = (field: string) => {
    // Ensure newOption[field] is defined and not an empty string
    if (newOption[field]?.trim()) {
      setCustomOptions((prev) => ({
        ...prev,
        [field]: [...(prev[field] || []), newOption[field]!], // Use non-null assertion (!) to tell TypeScript it's safe
      }));
  ...
  const { mutate: createDocument, isPending: isCreatingDocument } = trpc.writerx.newCreateDocument.useMutation({
  ...
  const { mutate: createSocialDoc, isPending: isGeneratingSocialContent, isError } = trpc.writerx.createSocialDocument.useMutation({
    onSuccess(data) {
      setIsGeneratingAIResponse(false);
      // localStorage.setItem("AIResponse", aiResponse ?? "");
      utils.writerx.getAllDocs.invalidate();
  ...
  const validateInputParams = () => {
    ...
    return inputparams.every(param => {
      ...
      if (field.type === 'multiple') {
        ...
        const currentSelectedOptions = selectedOptions?.[field.label] ?? [];
        const isValid = Array.isArray(selectedOptions[field.label]) && currentSelectedOptions.length > 0;
        console.log(`Field "${field.label}" (multiple type) is valid:`, isValid);
        return isValid;
      }
  ...
  const handleGenerate = async () => {
    ...
    const text = inputdata && inputdata.length > 0
      ? formData[inputdata[0]?.split(":")[0] as string]
      : transcript;
      ...
      <div className="space-y-8">
        ...
        {!isInputDataComplete && (
          <Button
            ...
            className="flex gap-1 py-1 px-2 rounded-lg"
          >
            {isTranscribing && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}
            {isTranscribing ? "Transcribing..." : "Next"}
            {!isTranscribing && <ChevronRight className="w-4 h-4" />}
          </Button>
        )}
          ...
          <>
            <div className="flex justify-start">
              <Button
                ...
                variant={"default"}
                className="mt-4 flex gap-1 py-1 px-2 rounded-lg"
              >
                <ChevronLeft className="w-4 h-4" /> Back
              </Button>
            </div>

            <div className="grid gap-5 tb:grid-cols-2">
                  ...
                  <div key={index} className="space-y-3">
                    ...
                    <ParameterField
                      ...
                      onShowNewOptionInput={(field, show) =>
                        setShowNewOptionInput(prev => ({ ...prev, [field]: show }))}
              ...
              <SelectedImagesPreview
                addTrigger={
                  <button
                    ...
                    className="w-28 h-28 bg-xw-card hover:bg-xw-card-hover rounded-xl border border-xw-border flex items-center justify-center"
            ...
            >
              {isGeneratingAIResponse ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
