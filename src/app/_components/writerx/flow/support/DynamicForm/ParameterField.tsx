  ...
  if (field.type === "single" && field.options) {
    return (
      <div className="space-y-3">
        <Select
          onValueChange={(value) => onInputChange(field.label, value)}
          value={formData[field.label]}
        >
          <SelectTrigger
          >
        ...
        <Select>
          ...
          <div className="truncate">
            {currentSelectedOptions.length > 0
              ? currentSelectedOptions.join(", ") // Now this is safe
              : "Select options"}
          </div>
          </SelectTrigger>
          <SelectContent>
            <div className="p-3 space-y-2">
              {[
                ...
                const isSelected = (selectedOptions[field.label] || []).includes(
                  option
                );
                return (
                  <div key={optIndex} className="flex items-center space-x-3">
                    <XWCheckbox
                      ...
                      className="h-5 w-5 border-2 border-gray-700/50 text-indigo-500 rounded-lg focus:ring-indigo-500 bg-gray-900/50"
                    />
                    <label
                      htmlFor={`${field.label}-${optIndex}`}
                    >
                      {option}
                    </label>
                  </div>
