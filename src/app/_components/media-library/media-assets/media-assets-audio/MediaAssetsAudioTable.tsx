...
import { columns } from "./MediaAssetsAudioColumn"; // Columns specific to audio
...
export default function MediaAssetsAudioTable() {
  ...
  const filteredAssets = useMemo(() => {
    ...
    return assets.audio.filter((audio) => {
  ...
  }, [assets?.audio, timeFilter]);

  const table = useReactTable({
    data: filteredAssets as unknown as MediaAssetsAudio[],
    columns,
  ...
  const deleteAudio = trpc.assets.deleteMultipleMediaAssets.useMutation({
    onSuccess: () => {
  ...
  const handleDeleteSelected = () => {
    if (selectedRowIds && selectedRowIds.length > 0) {
      ...
      deleteAudio.mutate({
        assetIds: selectedRowIds, // Pass selectedRowIds here
  ...
  return (
    <div className="w-full">
      {!isLoading && !isError && (assets?.audio?.length as number) > 0 && (
        <div>
          <div>
            <Table>
              <TableHeader>
