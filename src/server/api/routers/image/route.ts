...
export const imageRouter = createTRPCRouter({
  ...
  storeGeneratedImage: privateProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const { imageKey, prompt, generationType, workspaceId, resolution } =
        input;
      try {
      ...
      try {
        ...
        const signedUrls = await Promise.all(
          images.map(async (image) => {
            const key =
              image.imageKey?.split("/").pop()?.replace(".png", "") || "";
        ...
        return {
          images: signedUrls,
          nextCursor:
            images.length === limit ? images?.[images.length - 1]?.id : null, // For pagination
        };
      } catch (error) {
